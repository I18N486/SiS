...
public class ByteRingBuffer {
    ...
    public boolean get(ByteBuffer buff, int offset, int size) {
        if (buff != null && offset + size <= buff.capacity()) {
            buff.clear();
            if (this.getBusySize() < size) {
                return false;
            } else {
                ...
                buff.put(this.m_buffer,this.m_head,num_cpy);
                if (size - num_cpy > 0) {
                    buff.put(this.m_buffer,0,size - num_cpy);
                }
